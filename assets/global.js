...
document.addEventListener('click', function (e) {
  ...
  const toggle = e.target.closest('[data-toggle-menu-mb]');
  if (!toggle) return;

  e.preventDefault();

  const parentItem = toggle.closest('.menu-lv-1');
  ...
  document.querySelectorAll('.header__submenu.open').forEach(function (menu) {
    if (menu !== submenu) {
