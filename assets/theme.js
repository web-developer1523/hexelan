...
// (function() {
//   const sectionEl = document.getElementById('variant-faq-section');
//   if (!sectionEl) return;

//   // Find the product form element that dispatches variant changes (Dawn uses 'product-form' or 'variant-radios'/'variant-selects')
//   const productForm = sectionEl.closest('section')?.querySelector('product-form, form[action*="/cart/add"]') || document.querySelector('product-form, form[action*="/cart/add"]');

//   function getSelectedVariantId(form) {
//     // Dawn puts current variant id in a hidden input named 'id'
//     const idInput = form && form.querySelector('input[name="id"]');
//     return idInput ? idInput.value : null;
//   }

//   async function updateVariantFAQ(variantId) {
//     try {
//       const url = new URL(window.location.href);
//       // Ask Shopify to render this same section again (server will choose selected_or_first_available_variant based on current URL)
//       // Ensure the variant is in the URL so the server picks it up
...
//       // Parse the returned section HTML and extract our inner snippet
//       const temp = document.createElement('div');
//       temp.innerHTML = html;

//       const incoming = temp.querySelector('#variant-faq-section');
//       const current = document.getElementById('variant-faq-section');
//       if (incoming && current) {
//         current.replaceWith(incoming);
//       }
//     } catch (e) {
//       console.warn('FAQ variant update failed', e);
//     }
//   }

//   // Initial sync on load
//   const initialVariantId = getSelectedVariantId(productForm);
//   if (initialVariantId) updateVariantFAQ(initialVariantId);

//   // Listen for variant changes
//   document.addEventListener('change', (e) => {
//     // Trigger on option selectors inside the product form
//     if (!productForm || !productForm.contains(e.target)) return;
//     const vid = getSelectedVariantId(productForm);
//     if (vid) updateVariantFAQ(vid);
//   });

//   // Some themes dispatch a custom event with detail.variant
//   document.addEventListener('variant:change', (e) => {
//     const vid = e.detail?.variant?.id || getSelectedVariantId(productForm);
//     if (vid) updateVariantFAQ(vid);
//   });
// })();
