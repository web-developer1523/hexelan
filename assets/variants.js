class VariantSelects extends HTMLElement {
    ...
    onVariantInit(){
        ...
        this.updateVariantStatuses();
         // ADDED: Append variant ID to combined_variant div during initialization
         this.appendVariantIdTocombined_variant();
    }
    ...
    onVariantChange(event) {
        ...
        if (!this.currentVariant) {
            ...
            this.checkQuantityWhenVariantChange();
                    // ADDED: Append variant ID to combined_variant div when variant changes
        this.appendVariantIdTocombined_variant();
        }
    }
    // ADDED: New method to handle appending variant ID
    appendVariantIdTocombined_variant() {
        // Find the div with ID "combined_variant"
        const combined_variantDiv = document.getElementById('combined_variant');
        
        if (combined_variantDiv && this.currentVariant) {
            // Clear existing content and append the current variant ID
            combined_variantDiv.textContent = `${this.currentVariant.id}`;
            
            // Alternative: If you want to keep adding/updating without clearing:
            // combined_variantDiv.textContent = `Current Variant: ${this.currentVariant.id}`;
            
            // Alternative: If you want to append multiple IDs as they change:
            // const span = document.createElement('span');
            // span.textContent = `${this.currentVariant.id}, `;
            // combined_variantDiv.appendChild(span);
        } else if (combined_variantDiv && !this.currentVariant) {
            // Handle case when no variant is selected
            combined_variantDiv.textContent = 'No variant selected';
        }
    }
    updateOptions() {
